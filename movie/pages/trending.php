...
<body class="flex h-screen overflow-hidden">
    ...
    <div id="sidebar" class="fixed inset-y-0 left-0 w-64 bg-[#0b0b0b] border-r border-gray-800 z-50 transform -translate-x-full transition-transform duration-300 flex flex-col md:relative md:translate-x-0">
        ...
        <nav class="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
            ...
            <a href="trending.php" class="block px-4 py-3 rounded bg-red-600/10 text-red-500 border-l-4 border-red-600 font-bold">🔥 Trending</a>
            ...
            <a href="upcoming.php" class="block px-4 py-3 rounded text-gray-400 hover:bg-gray-800 hover:text-red-500 transition font-bold">🚀 Upcoming</a>
        </nav>
    </div>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-y-auto">
        <header class="p-6 border-b border-gray-800 bg-[#0f0f0f] sticky top-0 z-30">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-xl font-black red-text uppercase tracking-widest">Trending Now</h1>
            </div>
        </header>

        <div class="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4" id="grid">
            </div>
    </div>
    ...
    <script>
        ...
        async function loadContent() {
            ...
            grid.innerHTML = data.results.map(item => `
                <div class="cursor-pointer transition-all duration-300 rounded-lg overflow-hidden poster-hover relative bg-gray-900" onclick="window.location.href='watch.php?id=${item.id}&type=${item.media_type || 'movie'}'">
                    <div class="aspect-[2/3]">
                        <img src="https://image.tmdb.org/t/p/w500${item.poster_path}" class="w-full h-full object-cover">
                    </div>
                    <div class="p-3 bg-gradient-to-t from-black to-transparent">
                        <p class="text-xs font-bold truncate">${item.title || item.name}</p>
                        <span class="text-[10px] text-red-500">★ ${item.vote_average.toFixed(1)}</span>
                    </div>
                </div>
            `).join('');
        }
