...
<body class="flex h-screen overflow-hidden">
    ...
    <div id="sidebar" class="fixed inset-y-0 left-0 w-64 bg-[#0b0b0b] border-r border-gray-800 z-50 transform -translate-x-full transition-transform duration-300 flex flex-col md:relative md:translate-x-0">
        ...
        <nav class="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
            ...
            <a href="popular-tv.php" class="block px-4 py-3 rounded bg-red-600/10 text-red-500 border-l-4 border-red-600 font-bold">📺 Popular TV</a>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-y-auto">
        <header class="p-6 border-b border-gray-800 bg-[#0f0f0f] sticky top-0 z-30">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-xl font-black red-text uppercase tracking-widest">Popular Shows</h1>
            </div>
        </header>
    ...
    <script>
        ...
        async function loadContent() {
            ...
            grid.innerHTML = data.results.map(item => `
                <div class="cursor-pointer transition-all duration-300 rounded-lg overflow-hidden poster-hover relative bg-gray-900" onclick="window.location.href='watch.php?id=${item.id}&type=tv'">
                    <div class="aspect-[2/3]">
                        <img src="https://image.tmdb.org/t/p/w500${item.poster_path}" class="w-full h-full object-cover" alt="${item.name}">
                    </div>
